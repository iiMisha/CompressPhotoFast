...
- Умный запуск: скилл `/test-runner` (используй этот!)
- ⛔ НЕ ИСПОЛЬЗУЙ: `/android-test-suite` (вызывает этот агент → рекурсия!)

**🚨 КРИТИЧЕСКОЕ ПРАВИЛО:** Этот агент НЕ должен запускать тесты напрямую. Только анализировать покрытие и давать рекомендации. Для запуска тестов используй `/test-runner`.
