...
Когда пользователь запрашивает инициализацию memory bank (команда `initialize memory bank`), я выполню исчерпывающий анализ проекта, используя **ПРЯМЫЕ ИНСТРУМЕНТЫ** (НЕ Task(Explore) - вызывает переполнение памяти!):
...
На основе собранной информации создайте/обновите:
...
**НИКОГДА** не используйте Task(Explore) - это вызывает переполнение памяти JavaScript heap!
