...
**ВАЖНО: Запуск тестов выполняется ТОЛЬКО в самом конце завершённой задачи**, после всех изменений кода и успешной сборки.

### Создание тестов
При разработке создавайте необходимые unit и инструментальные тесты, но НЕ запускайте их до завершения всей задачи.

### Запуск тестов (в КОНЦЕ задачи)

**ВАЖНО: При запуске тестов через Bash tool ВСЕГДА указывайте timeout=1200000 (20 минут):**
...
**ПРИМЕР правильного вызова через Bash tool:**
- При использовании Bash tool для запуска тестов ВСЕГДА добавляйте параметр `timeout=1200000`
- Это предотвратит прерывание тестов из-за истечения времени ожидания агента
- Unit тесты выполняются ~10 минут
- Instrumentation тесты выполняются ~15 минут
- Все тесты вместе: ~20-25 минут
...
### Локальные агенты проекта

В папке `.claude/agents/` находятся локальные копии специализированных агентов, адаптированных для проекта:

#### Языковые специалисты:
- **`kotlin-specialist`** - Kotlin/Android разработка (Jetpack Compose, корутины, KMP, Room)
- **`java-architect`** - Java код и Android SDK архитектура

#### Инфраструктурные специалисты:
- **`deployment-engineer`** - CI/CD, Gradle, GitHub Actions
- **`devops-engineer`** - Автоматизация процессов
- **`platform-engineer`** - Инструменты разработки и DX
- **`database-administrator`** - Room DB, SQLite оптимизация

**Документация:** См. `.claude/agents/README.md` для подробного описания каждого агента.

**Примечание:** Локальные копии служат для быстрого доступа к документации и возможной адаптации под проект. Агенты вызываются через глобальные префиксы `voltagent-lang:` и `voltagent-infra:`.
