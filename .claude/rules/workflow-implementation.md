...
```
Task(Explore, "medium", "Найти существующие реализации [компонент/паттерн]

Нужно понять:
1. Какие паттерны используются
2. Как структурирован код
3. Какие библиотеки и зависимости применяются
4. Какие best practices есть в проекте
5. Как организована архитектура

Цель: Реализовать [новая функция] в том же стиле.")
```
...
**Workflow:**
1. `Task(Explore, "medium", "Найти похожие функции")`
...
**Workflow:**
1. `Task(Explore, "medium", "Найти место с багом и понять контекст")`
2. Прочитать Memory Bank (особенно context.md)
3. `Task(voltagent-lang:kotlin-specialist, "Исправить баг")`
4. `./gradlew assembleDebug`
5. (Опционально) Написать regression тест

### Рефакторинг

**Workflow:**
1. `Task(Explore, "very thorough", "Проанализировать весь модуль")`
...
3. `./gradlew test` или `./gradlew connectedAndroidTest`
...
```
// Шаг 1: Исследование
Task(Explore, "medium", "Найти реализации ViewModel")
...
### Связь с .kilocode/rules/rules.md
...
- [ ] Task(Explore) выполнен для исследования паттернов
...
**ПРАВИЛЬНО:**
```
Сначала: Task(Explore, "medium", "Найти паттерны")
