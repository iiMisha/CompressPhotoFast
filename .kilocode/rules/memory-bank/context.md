# Контекст

## Последние изменения
*   **Обновление memory bank (09.02.2026)**: Проверена актуальность всех файлов memory bank. Подтверждены версии зависимостей и структура проекта.
*   **Обновление зависимостей (февраль 2026)**: Обновлен Android Gradle Plugin до 9.0.0, KSP до 2.3.2, базовая версия приложения до 2.2.10. Добавлены новые зависимости: AndroidX Core KTX 1.17.0, AndroidX Lifecycle 2.9.2, AndroidX Activity 1.10.1, Hilt Work 1.2.0, AppCompat 1.7.1.
*   **Переход на экономичный режим (22.01.2026)**: Проект настроен на использование экономичного режима Gradle по умолчанию для снижения нагрузки на CPU. Документация: [`docs/GRADLE_ECO_MODE.md`](docs/GRADLE_ECO_MODE.md).
*   **Расширенная система тестирования (январь 2026)**: Новые скрипты (quick_test.sh, run_e2e_tests.sh, run_performance_tests.sh, start_emulator.sh, check_device.sh, generate_test_images.sh), базовые классы для тестов (BaseUnitTest, BaseInstrumentedTest, CoroutinesTestRule).
*   **Скрипты установки CLI**: Автоматические установщики для Linux/macOS и Windows с детекцией Python.
*   **Тестирование HEIC форматов**: Созданы специальные тесты для проверки обработки HEIC/HEIF форматов (20 тестов).

## Текущее состояние проекта

### Android-версия
*   Версия: 2.2.10 (текущая), versionCode 2
*   Kotlin: 2.2.10, Android Gradle Plugin: 9.0.0
*   DataStore для хранения настроек
*   Автоматическое сжатие в фоновом режиме
*   Игнорирование фото из мессенджеров и скриншотов
*   JaCoCo coverage: ~8-10% (целевой 50-70%)
*   **30 утилит** в пакете `util`
*   **Unit тесты**: 24 тестовых файла в `app/src/test/`
*   **Instrumentation тесты**: 18 тестовых файлов в `app/src/androidTest/`

### CLI-версия
*   Версия: 1.0.0
*   Python: 3.10+
*   9 модулей в `compressphotofast-cli/src/`
*   Многопроцессорная обработка (ProcessPoolExecutor)
*   Process-safe статистика (multiprocessing.Manager().Lock())
*   Поддержка HEIC/HEIF (pillow-heif)
*   Dry-run режим
*   Автоматические установщики для Linux/macOS и Windows

## Текущие проблемы

### 🔴 Дубликаты сжатых файлов при массовой обработке
**Описание**: При обработке 50+ файлов при включении автоматического сжатия с заменой создаются дубликаты сжатых файлов в отдельной папке приложения.

**Примеры имен дубликатов**:
- `IMG_20251012_150055_1_original.jpg`
- `IMG_20251012_150100_1_original.jpg`
- `IMG_20251012_173614_original.jpg`

**Подозрение**: Проблема связана с доступом к файлам через URI, из-за чего не происходит копирование в исходную директорию.

**Документация**: [`.docs/TASKS.txt`](../.docs/TASKS.txt)

### 🔴 Двойные расширения файлов
**Описание**: Создаются файлы с двойными расширениями (например, `image.HEIC.jpg`).

**Текущее поведение**:
- MIME тип всегда hardcoded как `"image/jpeg"`
- Двойные расширения не очищаются
- Исходный формат (HEIC/HEIF) игнорируется

**Ожидаемое поведение**:
- Чистые имена файлов (одно расширение)
- Правильный MIME тип для каждого формата
- Сохранение метаданных исходного файла

## Дальнейшие шаги
*   Увеличить coverage тестов до 50-70%
*   Добавить instrumentation тесты для оставшихся компонентов
*   Исправить проблему двойных расширений файлов
*   Исправить проблему с дубликатами сжатых файлов при массовой обработке
*   Реализовать массовую обработку через выбор папки (`.docs/fix_plan.md`)
*   Документировать повторяющиеся задачи в tasks.md
*   Реализовать E2E тесты для всех категорий
*   Добавить performance тесты
*   Интегрировать автоматический запуск эмулятора в CI/CD пайплайн
