...
*   **Многопоточная обработка**: Использование ThreadPoolExecutor для параллельной обработки изображений с автоматическим определением количества потоков на основе CPU ядер
*   **File-based locking**: Механизм блокировки файлов для предотвращения race conditions при многопоточной обработке
*   **Thread-safe статистика**: Безопасный сбор статистики сжатия из нескольких потоков
...
*   **Dry-run режим**: Предварительный просмотр сжатия без фактической обработки с многопоточной анализом
