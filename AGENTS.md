...
- 📌 **Постоянная служба обнаружения/сжатия фото (foreground service specialUse)** (Android) — по плану `.kilo/plans/1785676929231-persistent-photo-monitoring-service.md`. Цель: чтобы Android не закрывал процесс обнаружения/сжатия новых фото при включённом автосжатии. `BackgroundMonitoringService` переведён с типа `dataSync` (лимит ~6ч/сутки на Android 14+) на `specialUse` (без лимита времени) + `PROPERTY_SPECIAL_USE_FGS_SUBTYPE`; `startForeground` использует `FOREGROUND_SERVICE_TYPE_SPECIAL_USE` на API 34+, двухаргументный вызов ниже. Добавлен единый `MonitoringController` (start/stop) — используется из `MainActivity`/`MainViewModel`/`BootCompletedReceiver` вместо дублированного запуска; `BootCompletedReceiver` теперь также обрабатывает `MY_PACKAGE_REPLACED` (восстановление после обновления APK). Усилен сервис: `START_STICKY`, флаг `isUserStopped` (не восстанавливаться после явной остановки), `onTaskRemoved` перепланирует резервный Job, `ACTION_STOP_SERVICE` отменяет Job. `ImageDetectionJobService.cancelJob()` добавлен для чистой остановки. Foreground-тип WorkManager-задач (`createForegroundInfo`/`createSilentForegroundInfo`) изменён на семантически корректный `mediaProcessing` (API 34+) / `dataSync` (ниже). Добавлен `BatteryOptimizationHelper` + настройка `PREF_BATTERY_EXEMPTION_REQUESTED`: при первом включении автосжатия `MainActivity` запрашивает исключение из оптимизации батареи (Doze) через системный диалог (однократно). Разрешения: `FOREGROUND_SERVICE_SPECIAL_USE`, `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS`. Гарантии: обычное завершение процесса и перезагрузка после разблокировки восстанавливают мониторинг автоматически; force stop / отзыв разрешений / ручное ограничение батареи программно не обходятся. Верификация: `assembleDebug` зелёная, `compileDebugAndroidTestKotlin` зелёная, 339 unit-тестов (335 проходят + 4 пропуска perf) проходят, новый `MonitoringControllerTest` (Robolectric) + тесты battery-флага `SettingsManager`. Instrumentation-тесты требуют эмулятор `Small_Phone` (не запускались — нет подключённого устройства).
...
### Метрики
- Исходный код: 35 Kotlin-файлов, 9 Python-файлов
- Покрытие тестами: 339 Unit-тестов (335 проходят + 4 пропуска perf), 248 Instrumentation-тестов
- Версия: 2.2.10
