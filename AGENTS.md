...
- Перед рефакторингом искать дублирование и мертвый код навыком `code-analyzer`.
- После изменения Android-кода собрать приложение; тесты запускать согласно разделу «Проверка и релиз».
- После значимых изменений обновлять этот файл навыком `agents-updater`; держать его кратким и актуальным.
