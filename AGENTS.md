...
- Маркировать сжатые файлы EXIF-тегом `CompressPhotoFast_Compressed:quality:timestamp:size` (size — фиксированной ширины, двухфазная запись в `ExifUtil`; размер в маркер пишется итеративно через fstat-сверку — `saveAttributes()` может менять размер файла, при нестабильности маркер сбрасывается на заглушку).
- Повторная обработка файла с маркером — только если текущий размер файла отличается от size в маркере; mtime не используется.
