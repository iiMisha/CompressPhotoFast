...
**ВАЖНО**: Разработка ведется в среде Linux. Всегда отвечать на русском языке и использовать русские комментарии в коде.
...
Проект использует JUnit для юнит-тестирования и Espresso для инструментального тестирования, как настроено в `app/build.gradle.kts`.