...
class ManualCompressionE2ETest : BaseE2ETest() {
     ...
     */
    @Test
    fun testCompressionReducesFileSize() {
        runBlocking {
            if (testUris.isEmpty()) {
                return@runBlocking
            }
            ...
            val uri = testUris[0]
            val originalSize = runBlocking { UriUtil.getFileSize(context, uri) }
            ...
            val compressedSize = runBlocking { UriUtil.getFileSize(context, result.second!!) }
     ...
     */
    @Test
    fun testCompressionCompletionNotification() {
        runBlocking {
            if (testUris.isEmpty()) {
                return@runBlocking
            }
     ...
     */
    @Test
    fun testAlreadyCompressedFileSkipped() {
        runBlocking {
            if (testUris.isEmpty()) {
                return@runBlocking
            }
    ...
    fun testSmallFileSkipped() {
        runBlocking {
            ...
            val fileSize = runBlocking { UriUtil.getFileSize(context, smallUri) }
     ...
     */
    @Test
    fun testCompressionWithLowQuality() {
        runBlocking {
            if (testUris.isEmpty()) {
                return@runBlocking
            }
     ...
     */
    @Test
    fun testCompressionWithHighQuality() {
        runBlocking {
     ...
     */
    fun createTestImage(width: Int, height: Int): Uri? {
        return try {
